...
with st.sidebar:
    ...
    st.markdown("**Google AI Studio API 키**")
    ...
    if api_key:
        ...
        st.info("aistudio.google.com에서 무료로 발급받을 수 있어요")
...
with col_right:
    ...
    if not chat_msgs:
        chat_html += (
            ...
            '왼쪽 시작 문장을 복사해서 아래 채팅창에 붙여넣어 보세요</div>'
        )
    else:
        for msg in chat_msgs:
            if msg["role"] == "user":
                chat_html += (
                    ...
                    f'<div class="user-bubble" style="background:{color}">'
                ...
                bubble_class = "guide-bubble" if is_guide else "ai-bubble"
                prefix = "🟡 " if is_guide else ""
                chat_html += (
                    f'<div class="ai-msg">'
                    f'<div class="ai-avatar">{"🌿" if not is_guide else "📢"}</div>'
                    f'<div class="{bubble_class}">'
                    f'{prefix}{msg["content"].replace(chr(10),"<br>")}</div>'
                    f'</div>'
                )
    chat_html += '</div>'
    ...
    if user_input:
        if not api_key:
            st.error("왼쪽 사이드바에서 Google AI Studio API 키를 먼저 입력해주세요!")
        else:
            ...
            try:
                ...
                # Gemini 형식으로 대화 기록 변환
                history = []
                msgs = st.session_state[f"chat_{cur}"]
                for m in msgs[:-1]:  # 마지막 사용자 입력 제외
